...
public class GuiScreenStalkerModPlayerOptions extends AbstractTAEGuiScreen {
	...
	public GuiScreenStalkerModPlayerOptions(GuiScreen parent, String uuid, ResourceLocation skin, int skinwidth, int skinheight) {
    	...
    	updatename = PlayerAlert.needsUpdate(uuid);
    	
 		toggleAlert = new GuiOnOffButton(0, 0, 0, 135, 20, "Alert Notification ", PlayerAlert.joinAlert(uuid), "Alerts when online", 1);
 		toggleQueue = new GuiOnOffButton(1, 0, 0, 135, 20, "Queue Notification ", PlayerAlert.queueAlert(uuid), "Alerts when in queue", 1);
 		assumePrio = new GuiOnOffButton(2, 0, 0, 135, 20, "Assume Priority ", PlayerAlert.assumePrio(uuid), "Make queue position based on prio", 1);
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if(button == addColorCode) {
			...
			if(button == toggleAlert) {
				...
				PlayerAlert.updateSendAlert(uuid, ((GuiOnOffButton) button).getValue());
		
			} else if(button == toggleQueue) {

				PlayerAlert.updateQueueSendAlert(uuid, ((GuiOnOffButton) button).getValue());
		
			} else if(button == assumePrio) {
				
				PlayerAlert.updateAssumePrio(uuid, ((GuiOnOffButton) button).getValue());
