...
public class GuiScreenVisualizePackets extends AbstractTAEGuiScreen {
    ...
    private static final ModifiedList<GuiExtendButton> extendButtonsToDraw = new ModifiedList<>();
		...
		if(size > 0) {
			...
			do {
				...
				if(curr.y + curr.getHeight() <= j + 120 && curr.y >= j - 120) {
					...
					GuiExtendButton button1 = curr.extendButton();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		for(GuiExtendButton button : extendButtonsToDraw) {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if(button instanceof GuiExtendButton) {
			((GuiExtendButton) button).changeStateOnClick();
